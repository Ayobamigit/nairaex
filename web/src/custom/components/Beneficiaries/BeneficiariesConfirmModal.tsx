...
const BeneficiariesConfirmModalContentComponent: React.FC<Props> = ({ description, beneficiary, handleToggleConfirmationModal, clearInputs }: Props) => {
    ...
    const isWarningName = React.useMemo(() => (fullName.toLowerCase().trim() !== `${lookup?.first_name} ${lookup?.surname}`.toLowerCase() || !lookup?.first_name ||  !lookup?.surname), [lookup, fullName]);
    ...
    const validateFullName = React.useMemo(() => fullName.length > 40 || fullName.trim().match(/^(?:[\u00c0-\u01ffa-zA-Z'-]){2,}(?:\s[\u00c0-\u01ffa-zA-Z'-]{2,})+$/i) === null, [fullName]);
